...
class Pagination(object):
    def __init__(self, request, queryset, page_param='page', goto_page_param='goto_page', page_size=10, plus=5):
        ...
        # # 在点击分页时，实现保留搜索条件
        self.query_dict = copy.copy(request.GET)
        # 下面这条语句很关键，可以避免在goto_page_param和page两条key在请求中同时生效引起的冲突
        # 直接将goto_page从后面点击页码标签超链接中剔除
        self.query_dict.pop(goto_page_param, None)
        ...
        self.goto_page_param = goto_page_param
    ...
    def html(self):
        ...
        page_str_total.append('<li class="page-item"><a class="page-link" href="?{}" '
                              'aria-label="Previous">'
        ...
        # 下一页
        self.query_dict.setlist(self.page_param, [self.page + 1 if self.page != self.page_amount else self.page_amount])
        page_str_total.append('<li class="page-item"><a class="page-link" href="?{}" '
                              'aria-label="Next">'
                              '<span aria-hidden="true">&raquo;</span></a></li>'.format(self.query_dict.urlencode()))

        # 尾页
        self.query_dict.setlist(self.page_param, [self.page_amount])
        page_str_total.append(
        ...
        gotopage_str.append(
            ...
            '<input type="text" placeholder="页码" class="page-link" name="{}" value="{}">'
            '<button class="btn btn-outline-primary " type="submit">跳转</button>'
            '</form>'
            '</li>'.format(self.goto_page_param, self.goto_page)
        )
